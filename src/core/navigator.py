...
class VerseNavigator(QWidget):
    ...
    def __init__(self, db, display, parent=None, push_callback=None, clear_callback=None):
        ...
        self._push_callback = push_callback or (lambda v: logger.error("VerseNavigator: no push callback configured"))
        self._clear_callback = clear_callback or (lambda: logger.error("VerseNavigator: no clear callback configured"))
    ...
    def _on_select(self, idx, event):
        ...
        if self._state == 2:
            ...
            self._push_callback(clicked_verse)
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key.Key_Down:
            ...
            if new_idx < len(self.verses):
                ...
                if self._state == 2:
                    self._push_callback(self.verses[new_idx])
