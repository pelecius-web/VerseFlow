...
class DisplayChannel(QObject):
    ...
    def display_window(self):
        """Forwarding property — avoids NDISender reaching into controller internals."""
        return getattr(self._controller, 'display_window', None)
    ...
    def push_verse(self, verse: dict):
        ...
        if hasattr(self._controller, 'push_verse'):
            ...
            if hasattr(self._controller, "_pending_display_mode"):
                self._controller._pending_display_mode = self._display_mode
    ...
    def set_display_mode(self, mode: str):
        ...
        if display_window is not None and hasattr(display_window, "set_display_mode"):
            recreate = getattr(self._controller, "recreate_display_window_for_mode", None)
            if getattr(display_window, "_is_live", False) and recreate is not None:
                recreate(mode)
            else:
    ...
    @property
    def theme(self):
        """The current per-channel Theme object, or None."""
        return getattr(self, '_theme', None)

    def set_theme(self, theme):
        """Apply a per-channel Theme object.

        Caches the theme and forwards to DisplayWindow if it exists.
        Uses self._controller.set_theme() (forwarding chain) — never
        calls QApplication.setStyleSheet() (that's set_app_theme only).
        """
        self._theme = theme
        dw = getattr(self._controller, 'display_window', None)
    ...
    def apply_settings(self, settings: dict):
        ...
        if theme_id and getattr(self._controller, 'theme_mgr', None) is not None:
            theme = self._controller.theme_mgr.get_theme(theme_id)
            if theme is not None:
                self._unknown_theme_warned = False
                self.set_theme(theme)
            else:
                if not getattr(self, '_unknown_theme_warned', False):
                    logger.warning("[DisplayChannel] '%s': unknown theme_id '%s', falling back to 'dark_gold'",
                                   self.name, theme_id)
                    self._unknown_theme_warned = True
                fallback = self._controller.theme_mgr.get_theme("dark_gold")
