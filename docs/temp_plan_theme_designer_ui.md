# Phase 2 — Theme Designer UI
## Implementation Plan (v1.0)

> **Status:** Planned — ready for execution after Phase 1.5 patches sign-off
> **Sub-roadmap reference:** `docs/v1.3.0 Theme Designer — Sub-Roadmap.md` § Phase 2
...
### Deviation 1 — Property scope is the curated 33 keys that DisplayWidget actually consumes

**Sub-roadmap says:** "All theme JSON properties from schema v2 are editable."

**Plan says:** The designer exposes only the **33 properties** that `DisplayWidget` actually reads in rendering (after Phase 1.5 patches B1/B2 land). The remaining ~70 keys (`colors.bg_sidebar`, `colors.crossref_*`, `colors.draft_*`, `colors.scrollbar`, etc.) drive `generate_stylesheet()` for the operator panel, not the congregation display. Per Decision 3 (display-only theme scope), the operator panel chrome is controlled by `theme.active_theme` + `set_app_theme()` — outside this designer's responsibility. The 2 `lower_third.transition.*` keys are deferred to Phase 3 (when fade-transition rendering ships) and are NOT included in this scope.

**Justification:** Exposing 100 properties means ~70 dead rows where edits never affect the preview — a UX bug, not a feature. The 33-property scope is "what you see is what the congregation gets." A future "App Theme Designer" can address the operator panel keys separately if needed.

### Deviation 2 — Designer mounts on `MainWindow.stack` as index 2 (not a `QDialog`)
...
### Deviation 3 — Sample verses are hardcoded constants, not DB queries

**Sub-roadmap says:** "Designer holds these in a constant — does not query DB."
...
- `src/ui/theme_designer.py` — **NEW:** ~600 lines
  - `THEME_EDITOR_SCHEMA: list[PropertySpec]` — 33 properties, JSON-path-mapped
  - `SAMPLE_VERSES: list[dict]` — 4 hardcoded verse dicts
  - `BUILTIN_THEME_IDS: set[str]` — `{"dark_gold", "light", "high_contrast"}`
  - `class PropertySpec` — descriptor: `(json_path, label, widget_kind, options, tooltip)`
  - `class ThemesListPanel(QFrame)` — left column, theme list + buttons
  - `class PreviewSurface(QFrame)` — center, embedded `DisplayWidget` + mode toggle + verse picker + Apply controls
  - `class PropertyEditor(QScrollArea)` — right column, generated rows
  - `class ThemeDesignerPanel(QWidget)` — top-level, three-column layout

## Files Modified

- `src/utils/theme.py`
  - `Theme.update(json_path: str, value)` — set a value at a dotted path on the in-memory theme
  - `Theme.deep_copy() -> Theme` — return a fully independent Theme instance for editor sandboxing
  - `Theme.save() -> bool` — write back to `self.source_path`; returns False for unset path or IO error
  - `Theme.save_as(path: Path, new_id: str, new_name: str) -> Theme` — write to a new path, return the saved Theme
  - `Theme.delete() -> bool` — `self.source_path.unlink()`; refuses if `id` in `BUILTIN_THEME_IDS`
  - `ThemeManager.reload_theme(theme_id: str) -> Theme | None` — re-read JSON, replace registry instance
  - `ThemeManager.create_theme(new_id: str, new_name: str, source: Theme) -> Theme` — duplicate from an existing theme
...
- `scripts/pre_commit_checks.py` — add `check_phase2_theme_designer()`
- `scripts/verify_critical_fixes.py` — add `=== Phase 2 Theme Designer ===` block
...
- `src/display/display_channel.py` — `set_theme()` is the public entry point used by Apply; no changes needed
- `src/core/channel_manager.py`
- `src/display/display_widget.py` — Phase 1 output; designer embeds it read-only
- `src/display/display_window.py`
- `src/utils/settings.py`
- `src/utils/themes/*.json` — no schema changes in Phase 2
...
|---|---|
| `DisplayWidget(display_controller, theme_manager, church_name, parent)` | ✅ Confirmed |
| `set_theme(theme: Theme) -> None` | ✅ Confirmed |
| `set_display_mode(mode: str) -> None` | ✅ Confirmed |
| `set_logo_path(path: str \| None) -> None` | ✅ Confirmed |
| `set_church_name(name: str) -> None` | ✅ Confirmed |
| `_deferred_render(verse: dict) -> None` | ✅ Confirmed |
...
## Step 1 — Theme Persistence API (`src/utils/theme.py`)

Phase 2 adds 7 new methods. None affect the existing `set_app_theme()` / `get_theme()` contract.

### 1.1 `Theme.update(json_path: str, value)` — dotted path setter
...
def update(self, json_path: str, value) -> None:
    ...
    target = self
    for part in parts[:-1]:
        # Walk into known sections only — colors, fonts, lower_third, fullscreen, animation, spacing
...
### 1.2 `Theme.deep_copy() -> Theme` — sandboxing for editor
...
def create_theme(self, new_id: str, new_name: str, source: Theme) -> Optional[Theme]:
    ...
    if not re.match(r'^[a-z0-9_-]+$', new_id):  # E3 fix: sanitize before path construction
        logger.warning("[ThemeManager] Invalid theme id '%s' — use lowercase letters, digits, _ or -", new_id)
        return None
...
## Step 2 — `THEME_EDITOR_SCHEMA` (33 properties)
...
THEME_EDITOR_SCHEMA: list[PropertySpec] = [
    ...
    # c('text_primary'), c('text_faint') in display_widget.py:948-984.
    ...
    PropertySpec("fullscreen.verse_font_weight",        "Verse weight",       "font_weight", "Fullscreen",
                 tooltip="Note: dynamic verse text is rendered with the default weight in v1.3.0 Phase 2. Full effect requires B3 patch (font-fitting refactor) — tracked for follow-on."),
    ...
    # Phase 2 editor would be misleading — operator edits would have no
    # visible effect until Phase 3 ships.
]
```

**Total: 33 properties** (5 colors + 10 fullscreen + 18 lower-third). Down from 35 because 2 `transition.*` rows are deferred to Phase 3 — they are populated in the JSON by `_upgrade_to_v2` but `DisplayWidget` does not consume them yet.
...
## Step 3 — `theme_designer.py` Implementation
...
### 3.2 `_DummyDisplayController(QObject)` — sandboxed signal source

The preview widget needs a controller-like object with the same signal surface as `DisplayController` (verse_changed, layout_changed, translations_changed) but no DB or display-window side effects. The class is self-contained — no prior art reference.
...
### 3.3 `ThemesListPanel(QFrame)` — left column (180px)

Public signals:
- `theme_selected = pyqtSignal(str)` — emits `theme_id`
- `new_requested = pyqtSignal()`
- `duplicate_requested = pyqtSignal(str)` — emits source `theme_id`
- `delete_requested = pyqtSignal(str)` — emits target `theme_id`
...
### 3.4 `PreviewSurface(QFrame)` — center column (expanding)

Layout (top-to-bottom):
- **Mode toggle row (32px):** `QButtonGroup` with two buttons — "Fullscreen" / "Lower-Third"
- **Sample-verse picker row (32px):** `QComboBox` with `SAMPLE_VERSES` references
- **Embedded preview (expanding):** `DisplayWidget(display_controller=self._dummy, theme_manager=theme_mgr, church_name="Sample Church")` — set sizing policy `QSizePolicy.Expanding`
- **Apply controls row (60px):** Target combo (`Main` / `Alt` / `Both`) + `Apply` button

Public methods:
- `set_theme(theme: Theme)` — call `self._display_widget.set_theme(theme)`, push current sample verse
- `set_display_mode(mode: str)` — call `self._display_widget.set_display_mode(mode)`, re-push current verse
- `refresh()` — re-apply current theme + verse (cheap repaint)

Internal state:
- `_dummy: _DummyDisplayController` — created once
- `_display_widget: DisplayWidget` — created once, lifetime tied to `PreviewSurface`
- `_current_verse: dict` — currently selected sample verse
...
### 3.5 `PropertyEditor(QScrollArea)` — right column (320px)
...
|---|---|---|---|
| `color` | `QPushButton` (swatch) | `clicked` → `QColorDialog` | `dialog.selectedColor().name()` |
...
Public signal:
- `property_changed = pyqtSignal(str, object)` — emits `(json_path, new_value)`

Public method:
- `load_theme(theme: Theme)` — populate every row from the theme's current values

### 3.6 `ThemeDesignerPanel(QWidget)` — top-level
...
```
QHBoxLayout (root)
├─ ThemesListPanel       (180px fixed)
├─ PreviewSurface         (expanding)
└─ PropertyEditor         (320px fixed)
```

Above the three columns, a 50px header row with: title "Theme Designer", "Save", "Save As", "Delete", "Back to Settings" (calls `parent.parent()._switch_tab(1)` via signal).

Header buttons:
- **Save** — disabled when (a) no `_active_theme`, (b) `_active_theme.id ∈ BUILTIN_THEME_IDS`, (c) any channel `is_live`. On click: `_active_theme.save()` → `theme_mgr.reload_theme(...)` → success toast.
- **Save As** — always enabled when an `_active_theme` exists. Prompts for new id/name via `QInputDialog`. On confirm: `theme_mgr.create_theme(new_id, new_name, source=_active_theme)` → reselect new theme.
- **Delete** — disabled for built-ins and when the theme is currently applied to any live channel. Prompts confirm. On confirm: `_active_theme.delete()` → `theme_mgr` removes from registry → list refreshes.
- **Back** — emits `back_requested = pyqtSignal()` connected by `MainWindow` to `_switch_tab(1)`.

Internal slot wiring:
- `themes_list.theme_selected` → `_on_theme_selected(theme_id)` deep-copies registry instance, populates editor, refreshes preview
- `property_editor.property_changed` → `_on_property_changed(path, value)` calls `_active_theme.update(path, value)`, marks dirty, calls `preview.refresh()`
- `preview.apply_requested` → `_on_apply(target)` looks up target channel(s), calls `channel.set_theme(_active_theme)`. Live-channel guard runs first.

Internal slot wiring:
- `themes_list.theme_selected` → `_on_theme_selected(theme_id)` — calls `_confirm_discard_changes()` first if `_dirty`; deep-copies registry instance, populates editor, refreshes preview
- `property_editor.property_changed` → `_on_property_changed(path, value)` calls `_active_theme.update(path, value)`, marks dirty, calls `preview.refresh()`
- `preview.apply_requested` → `_on_apply(target)` looks up target channel(s), calls `channel.set_theme(_active_theme)`. Live-channel guard runs first.
- `back_requested` signal → `_on_back_requested()` — calls `_confirm_discard_changes()` first if `_dirty`; then emits `back_requested`
...
def _on_theme_selected(self, theme_id: str):
    if not self._confirm_discard_changes():
        # Revert list selection to the current theme — don't switch
...
Save flow: if `_any_channel_live()` → show dialog → on `No`, abort. Apply flow: same.

Per Q2, **property editing is never gated**. Only Save and Apply are.

---

## Step 4 — `settings_panel.py` Integration
...
### 4.2 New button — placed in the Channel Settings card, above the per-channel rows
...
The current `_switch_tab` updates `btn_tab_home` and `btn_tab_settings` styles. There's no designer button on the Home sidebar — it's reached via Settings or shortcut. So no new button-style branch is needed in `_switch_tab`. The method just needs to handle index 2 without crashing:
...
def _switch_tab(self, index):
    ...
    # Index 2 (designer) — no sidebar button to highlight.
...
Add to `MainWindow.__init__`, after the `_hotkey_manager.start(self)` call (E4 fix: `Ctrl+L` lives in `home_panel.py:726`, not `MainWindow.__init__` — there is no Ctrl+L block in main.py to anchor against):
...
## Step 6 — Pre-Commit Guards (`scripts/pre_commit_checks.py`)
...
def _is_v2_default_path(path: str) -> bool:
    ...
    return path.startswith(("fullscreen.", "lower_third.transition.")) \
           or path in (
...
## Step 7 — Verification Guards (`scripts/verify_critical_fixes.py`)
...
- **Schema drift.** A new key added to `dark_gold.json` but not added to `THEME_EDITOR_SCHEMA` is invisible to operators. The reverse — schema entry without a matching JSON key — produces a broken row at runtime. Static guard runs path resolution against `dark_gold.json` after v2 upgrade.
- **NDI capture of designer preview.** The preview's `DisplayWidget` must not be in `ChannelManager`'s registry. Mitigation: designer creates the widget standalone, never calls `channel_manager.add_channel(...)` for it. Verified by inspection — designer never imports `add_channel`.
- **Built-in overwrite via direct file edit.** `Save` refuses built-ins; `Save As` to `themes/dark_gold.json` would overwrite the file. Mitigation: `save_as` rejects when target path already exists in `BUILTIN_THEME_IDS` set or matches a built-in's path.
...
- [ ] Designer opens via Settings → "Open Theme Designer" and via `Ctrl+Shift+T` and via View → Theme Designer.
...
- [ ] Switching themes with unsaved edits prompts "Discard changes?" — Cancel keeps current theme and edits intact. (E5 fix)
...
- [ ] Preview matches live display when the saved theme is applied to a real channel — confirm with `John 3:16` on Main fullscreen.
...
|---|---|
| _TBD_ | Phase 2 start. |
| _TBD_ | Step 1 done — Theme persistence API. |
| _TBD_ | Step 2 done — `THEME_EDITOR_SCHEMA`. |
| _TBD_ | Step 3 done — `theme_designer.py` core classes. |
| _TBD_ | Step 4 done — Settings integration. |
| _TBD_ | Step 5 done — `main.py` wiring. |
| _TBD_ | Step 6 done — pre-commit guards. |
| _TBD_ | Step 7 done — verification guards. |
